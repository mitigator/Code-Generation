...
const handleDownloadZipClient = async () => {
  ...
  try {
    ...
    // Convert the file tree to a flat array of files
    const flattenFileTree = (items, files = []) => {
      items.forEach(item => {
        if (item.type === 'file') {
          files.push({
            file_path: item.path,
          ...
          flattenFileTree(item.children, files);
        }
      });
      return files;
    };
  ...
  const renderProgressBar = () => {
    const steps = [
      { id: 'source', label: 'Source Files', completed: status.hasCodeGenOutput },
      { id: 'combined', label: 'Combined', completed: status.hasCodeRefinementInput },
      { id: 'refined', label: 'Refined', completed: status.hasCodeRefinement },
      { id: 'merged', label: 'Merged', completed: status.hasMergedCodeOutput }
    ];

    const activeStep = steps.findIndex(step => !step.completed);
    const progressPercentage = activeStep === -1 ? 100 : (activeStep / steps.length) * 100;
      ...
      <div className="w-full px-6 py-4">
        <div className="relative">
          ...
          <div className="absolute top-4 left-0 right-0 h-1 bg-gray-200 z-0">
            <div 
              className="h-1 bg-green-500 transition-all duration-500 ease-in-out" 
